-- DROP DATABASE IF EXISTS hasznaltufo;
-- CREATE DATABASE hasznaltufo;
-- USE hasznaltufo;

CREATE TABLE tipus (
	tipusId int primary key identity not null,
	nev nvarchar(20) not null,
	alapar nvarchar(20) not null
);

CREATE TABLE vevo (
	vevoId int primary key identity not null,
	nev nvarchar(20) not null,
	cim nvarchar(20) not null,
	megbizhato bit not null
);

CREATE TABLE ufo (
	ufoId int primary key identity  not null,
	tipus int foreign key references tipus(tipusId) not null,
	lopott bit not null,
	serules int not null,
